...
class CR:
    def __init__(self, dataframe, species, n_r=2, verbose=True):
        ...
        self.runODE  = jit(lambda t_eval, x, r0, params: odeint(self.system, jnp.concatenate((x[0], r0)), t_eval, params))
        ...
        self.runODEZ = jit(lambda t_eval, x, r0, params: odeint(self.aug_system, (jnp.concatenate((x[0], r0)), Y0, *Z0), t_eval, params))
        ...
        def runODEA(t_eval, zt, at, cr_params):

            # concatenate final condition
            xal = (zt, at, Lt)

            # solve ODE model
            xaL0 = odeint(self.adj_system, xal, t_eval, cr_params)
            ...
            return grads
        self.runODEA = jit(lambda t, xt, at, cr_params: runODEA(jnp.array([0., t]), xt, at, cr_params))
    ...
    def aug_system(self, aug_x, t, params):
        ...
        dZdt = [jnp.einsum("ij,j...->i...", Jx_i, Z_i) + Jp_i for Z_i, Jp_i in zip(Z, jacfwd(self.system, 2)(x, t, params))]

        return (dxdt, dYdt, *dZdt)

    # augmented system for adjoint sensitivity method
    def adj_system(self, aug_x, t, params):
        ...
        y_dot, vjpfun = vjp(lambda x, params: self.system(x, t, params), x, params)
        vjps = vjpfun(a)
    ...
    def jacobian_adj(self, params):
        ...
        for treatment, t_eval, Y_measured in self.dataset:
            ...
            # forward pass
            output = self.runODE(t_eval, Y_measured, r0, params)
